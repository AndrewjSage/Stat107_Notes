...
library(tidyverse)
library(knitr)
movies <- read.csv("https://raw.githubusercontent.com/AndrewjSage/Stat_107/main/Data/movies2018.csv")
...
```{r, echo=FALSE}
Dolphins <- read.csv("https://raw.githubusercontent.com/AndrewjSage/Stat_107/main/Data/Dolphins.csv")
```
...
To perform the simulation, we'll use the following steps. 
   ...
   1. Simulate flipping a coin that has a 50% chance of landing heads 33 times. In this case, heads represents the dolphins pressing the right button.   
...
```{r}
Babies <- read.csv("https://raw.githubusercontent.com/AndrewjSage/Stat_107/main/Data/Babies.csv")
```
...
6. If getting a proportion as extreme more more extreme than 0.267 is rare, then we have evidence against the null hypothesis.   
...
A 2004 study by Lange, T., Royals, H. and Connor, L. (https://www.tandfonline.com/doi/abs/10.1577/1548-8659(1993)122%3C0074%3AIOWCOM%3E2.3.CO%3B2) examined Mercury accumulation in large-mouth bass, taken from a sample of 53 Florida Lakes. 33 of the lakes were in Northern Florida and 20 were in Southern Florida. Location was determined relative to Interstate-50 that runs through the center of the state.
...
There are three factors that impact the p-values in a hypothesis test.  
...
```{r}
set.seed(07052023)
FloridaLakesB <- cbind(FloridaLakes, FloridaLakes) 
SimulateMean(x=FloridaLakesB$Mercury, mu = 0.5, alternative="two.sided", reps=10000)
```

**1.Size of the difference between the sample statistic and parameter value in the null hypothesis.** 

For example, suppose the sample mean had been 0.577 ppm, instead of 0.527. (Thus farther away from the value of $\mu=0.5$) in the null hypothesis.   

```{r}
set.seed(07052023)
FloridaLakesB <- FloridaLakes %>% mutate(Mercury=Mercury + 0.05)
SimulateMean(x=FloridaLakesB$Mercury, mu = 0.5, alternative="two.sided", reps=10000)
```
As the size of the difference increases, the p-value decreases.

This makes sense, as our sample statistic is farther away from what the null hypothesis said, making our data less consistent with the null hypothesis.   
...
As the sample size increases, the p-value decreases.

This makes sense, as larger samples give us more information, and thus stronger evidence against the null hypothesis.   

**3.Standard deviation of individual observations**

```{r}
set.seed(07052023)
FloridaLakesD <- FloridaLakes %>% mutate(Mercury = runif(53, -0.5, 1.5)) 
FloridaLakesD <- FloridaLakesD %>% mutate(Mercury = Mercury -(mean(Mercury)-mean(FloridaLakes$Mercury)))
SimulateMean(x=FloridaLakesD$Mercury, mu = 0.5, alternative="two.sided", reps=10000)
```


As the standard deviation increases, the p-value increases.

This makes sense, as a larger standard deviation means there is more variability and uncertainty in our data, giving us weaker evidence against the null hypothesis.   
...
In the previous section's investigation, we saw two different histograms. The first gives the distribution of the mercury levels of 53 lakes in our sample. This distribution is shown again, below.     
...
The second gives the distribution of the mean mercury concentration from many different samples of size 53. We could think of our sample as just one of the many making up this histogram. This distribution is called the **sampling distribution of the mean**.

The **sampling distribution** of a statistic is the distribution of that statistic across many different samples of a given size.   
...
Hist_S10 <- gf_dhistogram(~Mercury, data=data.frame(S10), border=0, fill="blue", color="black")%>%
  gf_labs(x="Mean Mercury", y="Frequency", title="n=10")  + xlim(c(0, 1.5)) + theme(axis.text.y=element_blank())
Hist_S30 <- gf_dhistogram(~Mercury, data=data.frame(S30), border=0, fill="blue", color="black")%>%
  gf_labs(x="Mean Mercury", y="Frequency", title="n=20")  + xlim(c(0, 1.5)) + theme(axis.text.y=element_blank())
Hist_S50 <- gf_dhistogram(~Mercury, data=data.frame(S50), border=0, fill="blue", color="black")%>%
  gf_labs(x="Mean Mercury", y="Frequency", title="n=50") + xlim(c(0, 1.5)) + theme(axis.text.y=element_blank())
...
The table shows the average standard deviation and standard error of the mean for samples of various size.   
...
The "theoretical" standard error of a sample mean is:

\[
\text{SE}(\bar{x}) = \frac{s}{\sqrt{n}},
\]

where $\bar{x}$ represents the sample mean, $s$ is the sample standard deviation and $n$ is the sample size.   

**Example:**  

For a sample of size 53, with standard deviation 0.341, 

\[
\text{SE}(\bar{x}) = \frac{0.341}{\sqrt{53}}=0.0468
\]
...
We've seen the sampling distribution for the mean of a single sample.   

We've actually worked with sampling distributions before in 3 other contexts, without using that name.  

**Sampling Distribution for a Difference in Means** (North vs South Lakes Example)
...
\[
\text{SE}(\bar{x}_1-\bar{x}_2) = \sqrt{\frac{s_1^2}{n_1}+\frac{s_2^2}{n_2}} = \sqrt{\frac{0.27^2}{33}+\frac{0.38^2}{20}} = 0.097
\]

where $\bar{x}_1$, $\bar{x}_1$ represent sample means, $n_1$ and $n_2$ represent sample sizes, and $s_1$ and $s_2$ represent sample standard deviations. 

**Sampling Distribution for a Proportion** (Dolphins Example)
...
\[
\text{SE}(\hat{p}) = \sqrt{\frac{\hat{p}(1-\hat{p})}{n}}=\sqrt{\frac{0.5(1-0.5)}{16}}=0.125,
\]

where $\hat{p}$ represents the sample proportion and $n$ represents the sample size.   

**Sampling Distribution for a Difference in Proportions** (Babies Example)
...
\[
\text{SE}(\hat{p}_1-\hat{p}_2) = \sqrt{\hat{p}(1-\hat{p})\left(\frac{1}{n_1}+\frac{1}{n_2}\right)}   \sqrt{\frac{28}{60}\left(1-\frac{28}{60}\right)\left(\frac{1}{30}+\frac{1}{30}\right)} = 0.1288
\]
...
### Distribution of Sample Mean for Skewed Data   {.smaller}
...
When the sample size is large enough, standardized statistics follow symmetric bell-shaped distributions. We can use these to approximate the distribution of a statistic when the null hypothesis is true, allowing us to obtain a p-value, without using simulation.   

The p-value represents the probability of obtaining a standardized statistic as extreme or more extreme than we did when the null hypothesis is true.  

Tests based on standardized statistics are sometimes called "theory-based" tests.   
...
```{r, include=FALSE}
Students <- read.csv("https://raw.githubusercontent.com/AndrewjSage/Stat_107/main/Data/Students.csv")
