...
knitr::opts_chunk$set(echo = TRUE, message=FALSE, warning=FALSE, fig.height = 2, fig.width = 6)
library(tidyverse)
library(ggformula)
library(gridExtra)
library(knitr)
...
1. Identify sources of sampling bias.   
2. Identify confounding variables and explain how they impact our ability to draw inferences about causal relationships.     
3. Distinguish between an observational study and an experiment.   
4. Explain how to design an experiment using the principles of experimental design (control, randomization, replication, blocking, blinding, and placebo), when appropriate.     
5. Identify the sample, population, statistic, and parameter of interest in a given study.  
6. Distinguish between categorical and quantitative variables.    
7. Interpret data graphics including bar graphs, histograms, boxplots, and scatterplots.      
8. Draw conclusions about measures of center and spread (mean, median, interquartile range, standard deviation) based on graphical representations of data.    
9. Draw conclusions about measures of center and spread based on written descriptions of data.    
10. Explain how characteristics of a data distribution (including skewness and presence of outliers) affect measures of center and spread.    

11. Use R to create informative data graphics (bar graph, histogram, boxplot, scatterplot).
...
kable(movies %>% filter(Genre %in% c("Comedy", "Drama")) %>% 
  group_by(Genre) %>% summarize(Mean=mean(IMDB) %>% round(2), 
                                      Median=median(IMDB),
                                      IQR=IQR(IMDB),
                                      StDev=sd(IMDB)%>% round(2)))
```
...
12. Identify the parameter of interest and sample statistic using words and symbols for categorical data with one or two samples.    
13. State appropriate null and alternative hypotheses, using both words and symbols for categorical data with one or two samples.            
14. Describe how to use simulation to test hypotheses for categorical data with one or two samples.        
15. Define a p-value in a given context (i.e. "The p-value represents the probability of ...") for categorical data with one or two samples.        
16. State what we should conclude from a p-value, in context, for categorical data with one or two samples.          
17. Draw data-driven conclusions by synthesizing information including study design, statistical significance, and practical importance, for categorical data with one or two samples.         

18. Use R to analyze categorical data.  
...
19. Identify the parameter of interest and sample statistic using words and symbols for quantitative data with one or two samples.        
20. State appropriate null and alternative hypotheses, using both words and symbols for quantitative data with one or two samples.                
21. Describe how to use simulation to test hypotheses involving one or two samples for quantitative data with one or two samples.            
22. Define a p-value in a given context (i.e. "The p-value represents the probability of ...")  for quantitative data with one or two samples.            
23. State what we should conclude from a p-value, in context, for quantitative data with one or two samples.              
24. Draw data-driven conclusions by synthesizing information including study design, statistical significance, and practical importance, for quantitative data with one or two samples.             

25. Use R to analyze quantitative data.  
...
```{r}
SimulateMean(x=(FloridaLakes%>%filter(Location=="N"))$Mercury,
             y=(FloridaLakes%>%filter(Location=="S"))$Mercury, 
             alternative="two.sided", reps=10000)
```
...
26. State how changes in sample size impact variability of sampling distributions, test statistics, p-values, evidence against the null hypothesis, and the width of confidence intervals.     
27. Explain the difference between the distribution of values in a sample and the sampling distribution for a statistic.    
28. Calculate and interpret standardized (z and t) statistics for categorical and quantitative data, with one or two samples.    
29. Determine whether conditions for using "theory-based" approaches like normal and t-distributions are met.    
30. Show how to calculate confidence intervals for means and proportions, and differences in means and proportions.      
31. Interpret confidence intervals in context.      
32. Determine whether or not a value will be in a confidence interval, based on a relevant p-value, and determine whether a p-value will be large or small, based on information about a confidence interval.   
33. Determine when to use a paired t-test and when to use a t-test for independent samples.    
34. Explain Simpson's paradox and how draw conclusions in situations involving it.  
35. Distinguish between statistical significance and practical importance and make appropriate decisions based on these concepts.    
36. Recognize situations where we should use multiple testing procedures such as the Bonferroni correction.     

37. Use R to analyze data using theory-based tests and confidence intervals.     
...
Hist_S100 <- gf_dhistogram(~Mercury, data=data.frame(S100), border=0, fill="blue", color="black")%>%
  gf_labs(x="Mercury", y="Frequency", title="n=100") + xlim(c(0, 1.5)) + theme(axis.text.y=element_blank())
Hist_S1000 <- gf_dhistogram(~Mercury, data=data.frame(S1000), border=0, fill="blue", color="black")%>%
  gf_labs(x="Mercury", y="Frequency", title="n=1000")  + xlim(c(0, 1.5)) + theme(axis.text.y=element_blank())
...
### Distributions of z and t-statistics
...
More data have been recorded in the last two years than all previous human existence (Forbes magazine). Data have been used to improve people's lives in a wide range of ways. Nevertheless, data can be easily misused in dangerous ways, either intentionally, or by accident. According to *Nature* more than 50% of scientific studies cannot be reproduced, and more than 25% using p < 0.05 cutoff produce false results. 

A 2016 statement from the American Statistical Association wrote: 
...
A famous quote attributed to Mark Twain (and others) says "There are three kinds of lies: lies, damned lies, and statistics." 

In this section, we examine several realistic scenarios in which data might be used to draw incorrect, and even dangerous conclusions, if we aren't careful.    
...
The table shows the number of patients who survived and the number who died at two different hospitals, A and B. 
...
We now break down survival rates by health of patients at the time of admission to the hospital .

Patients in good condition (non-life threatening illness or injury):

|             |  Survived | Died | Total |
|-----------|----------|-----------|
| Hospital A | 590 (98.3%)  |  10  (1.7%)   | 1000 |
| Hospital B | 870 (96.7%)  |  30  (3.3%)   | 1000 |

Patients in poor condition (serious, possibly life-threatening condition):

|             |  Survived | Died | Total |
|-----------|----------|-----------|
| Hospital A | 210 (52.5%)  |  190  (47.5%)   | 1000 |
| Hospital B | 30  (30%)  |  70 (70%)    | 1000 |

Which hospital would you recommend to a person you care about? Why?
...
### Multiple Testing - Breakfast Cereal and Sex at Birth   
...
Explain what is wrong with the way the researchers used p-values to come to this conclusion. Hint: it relates to this [XKCD comic](https://xkcd.com/882/). 


### Multiple Testing Error and Bonferroni Correction {-}

The breakfast cereal conclusion demonstrates a pitfall, known as multiple testing error. 

When we perform more than one test, it increases the likelihood of getting a small p-value just by chance, even if there is really no difference between the groups. 
...
### Statistical Significance and Practical Importance - Airline Flights
...
data(flights)
Flights1 <- flights %>% filter(origin %in% c("EWR", "LGA") & dest =="ORD" & !is.na(air_time))
rm(flights)
```

```{r, fig.height=3, fig.width=8, echo=FALSE, eval=FALSE}
h <- gf_histogram(data=Flights1, .~air_time | origin, color="black", fill="blue") 
b <- gf_boxplot(data=Flights1, air_time~origin) + coord_flip()
grid.arrange(h, b, ncol=2)
```

```{r, fig.height=3, fig.width=5, echo=FALSE}
library(knitr)
T <- Flights1 %>% group_by(origin) %>% summarize(Mean_Airtime = mean(air_time), SD = sd(air_time), n=n())
kable(T)
```
...
### Statistical Significance and Practical Importance - Smoking and Birthweight
...
```{r, fig.height=3, fig.width=8, echo=FALSE, eval=FALSE}
h <- gf_histogram(data=NCBirths, .~weight | habit, color="black", fill="blue", binwidth = 2) 
b <- gf_boxplot(data=NCBirths, weight~habit) + coord_flip()
grid.arrange(h, b, ncol=2)
```
...
gf_dist("t", df = 21,  plot_size = 1, geom="area", fill=~!abs(x)>3.98 ) + 
  ...
  geom_vline(xintercept=-3.98, colour="red") + guides(fill="none") + xlab("t")
```
...
gf_dist("t", df = 21,  plot_size = 1, geom="area", fill=~!abs(x)>0.93) + 
  ...
  geom_vline(xintercept=-0.93, colour="red") + guides(fill="none") + xlab("t")
```
...
38. Describe relationships between variables in a scatterplot, including strength, direction, and form, and identify outliers.          
39. Estimate the correlation coefficient, $r$, between variables shown in a scatterplot.  
40. Identify and interpret the slope, intercept, and coefficient of determination ($R^2$) for a linear regression model using R output.   
41. Predict the value of a response variable for a given value of the explanatory variable using a regression line.   
42. Recognize the risk of extrapolation and state when it is inappropriate to make predictions using a regression line.    
43. Explain how to use simulation to test hypotheses in regression.     
44. Define a p-value in regression context. (i.e. "The p-value represents the probability of ...")    
45. State what we should conclude from a p-value, in regression context.      
46. Interpret test statistics and p-values for the slope of a regression line, using R-output.   
47. Explain the difference between confidence intervals and prediction intervals and determine when it is appropriate to use each type.   
48. Interpret confidence intervals (for slope, intercept, and expected response) and prediction intervals in regression.   

49. Use R to perform linear regression and analyze results.    
50. Interpret regression coefficients (slopes and intercepts) in a multiple regression model using R output.       
51. Interpret t-statistics and p-values in multiple regression.    
52. Predict the value of a response variable using multiple regression output in R.    
...
```{r, echo=FALSE, fig.width=8, fig.height=4, fig.align='center', message=FALSE, warning=FALSE}
library(MASS)
library(ggplot2)
library(gridExtra)
set.seed(09182017)
...
The simulation is performed in R. 
...
```{r, fig.height=2.5, fig.width=5, include=FALSE, echo=FALSE}
library(mosaic)
data(SAT)
```
...
53. State the null and alternative hypotheses associated with Chi-Square and ANOVA F-tests.    
54. Interpret the p-value and draw conclusions from Chi-Square and ANOVA 
F-tests.    
...
Since we're performing three different tests, we need to use the **Bonferroni correction**. Thus, the original p-value would need to be less than 0.05/3 = 0.0167 in order for the Bonferroni corrected p-value to be less than 0.05, and the original p-value would have to be less than 0.10/3 = 0.033 in order for the Bonferroni corrected p-value to be less than 0.10.
